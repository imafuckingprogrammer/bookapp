...
BEGIN
  IF (TG_OP = 'INSERT' OR TG_OP = 'UPDATE') THEN
    UPDATE public.books
    SET 
      average_rating = (SELECT AVG(rating) FROM public.reviews WHERE book_id = NEW.book_id),
      total_ratings = (SELECT COUNT(*) FROM public.reviews WHERE book_id = NEW.book_id)
    WHERE id = NEW.book_id;
  ELSIF (TG_OP = 'DELETE') THEN
    UPDATE public.books
    SET 
      average_rating = COALESCE((SELECT AVG(rating) FROM public.reviews WHERE book_id = OLD.book_id), 0),
      total_ratings = (SELECT COUNT(*) FROM public.reviews WHERE book_id = OLD.book_id)
