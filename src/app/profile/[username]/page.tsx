...
export default function UserProfilePage({ params }: { params: Promise<{ username: string }> }) {
  ...
  const fetchTabData = useCallback(async (tab: string, userId: string) => {
    if (!userId) return;
    setTabDataLoading(true);
    try {
      let response: PaginatedResponse<any>;
      switch (tab) {
        case 'diary': response = await getUserReadBooks(userId); setUserDiary(response.items); break;
        case 'reviews': response = await getUserReviews(userId); setUserReviews(response.items); break;
        case 'lists': response = await getUserLists(userId); setUserLists(response.items); break;
        case 'watchlist': response = await getUserWatchlist(userId); setUserWatchlist(response.items); break;
        case 'likes': response = await getUserLikedBooks(userId); setUserLikedBooks(response.items); break;
        case 'owned': response = await getUserOwnedBooks(userId); setUserOwnedBooks(response.items); break;
        // case 'followers': response = await getUserFollowers(userId); setFollowers(response.items); break;
        // case 'following': response = await getUserFollowing(userId); setFollowingUsers(response.items); break;
        default: break;
      }
    } catch (error) {
        console.error(`Failed to fetch ${tab} data:`, error);
        toast({title: "Error", description: `Could not load ${tab} data.`, variant: "destructive"});
    } finally {
  ...
  useEffect(() => {
    if (profile?.id) {
        fetchTabData(currentTab, profile.id);
    }
  }, [profile, currentTab, fetchTabData]);
  ...
  const handleLikeReview = async (reviewId: string) => {
    ...
    try {
      ...
      if (profile?.id) fetchTabData('reviews', profile.id);
    } catch (error) {
  ...
  const stats = {
    booksRead: userDiary.length, // Or use a count from API if paginated
        ...
        {tabDataLoading ? (
            ...
            <TabsContent value="reviews">
              <div className="space-y-6">
                {userReviews.length > 0 ? userReviews.map(review => <ReviewCard key={review.id} review={review} onLikeReview={handleLikeReview} onReplyToReview={(id,text) => console.log(id,text)} />) 
